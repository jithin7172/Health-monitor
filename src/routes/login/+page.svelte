<script lang="ts">
    ...
    let googleAuthUrl = $page.data.authUrl;
    ...
    onMount(() => {
      ...
      // Set loaded state after short delay
      setTimeout(() => {
        isLoaded = true;
      }, 100);
    ...
    async function handleGoogleSignIn() {
      try {
        ...
        if (userEmail) {
          // Check if document exists in userdata collection
          ...
          if (!userDoc.exists()) {
            await setDoc(userDocRef, {
              email: userEmail,
              // Add any other default fields you want to include
              createdAt: new Date(),
            });
          }
        }
        ...
        >
          ...
          >
            <div class="max-w-2xl mx-auto space-y-4 lg:space-y-6">
              ...
              >
                <span
                  class="text-white/80 w-24 lg:w-56 text-lg lg:text-2xl hero-text text-right font-sh-grotesk"
                >
                ...
                <span
                  class="text-white/80 w-24 lg:w-56 text-lg lg:text-2xl hero-text text-left font-sh-grotesk"
                >
        ...
        >
          <div class="w-full max-w-md space-y-6 lg:space-y-8 min-w-[280px] px-4">
            ...
            >
              ...
              <p
                class="mt-2 text-sm lg:text-base text-gray-600 dark:text-gray-400"
              >
                You can login or sign up here with multiple different client
                options
              </p>
            </div>
